...
for i in range(0, 5):
    # baseline_query = "java -jar bin/RankLib-2.15.jar -train own/iter4/fold{0}/train.txt -test own/iter4/fold{0}/test.txt -validate own/iter4/fold{0}/vali.txt -ranker 1 -metric2t NDCG@10 -save ndcg10baselinefold{0}.txt -epoch 100 -layer 1 -node 10".format(i)
...
for i in range(0, 5):
    # baseline_score_query = "java -jar bin/RankLib-2.15.jar -load ndcg10baselinefold{0}.txt -test own/iter4/fold{0}/test.txt -metric2T NDCG@10 -idv output/ndcg10baselinefold{0}output.txt".format(i)
